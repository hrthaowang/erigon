...
func TestCallIntraBundle(t *testing.T) {
	var (
	...
	//submit 3 txs and commit the results
	...
	timeout := int64(50000)
	res, err := api.CallIntraBundle(ctx, []Bundle{{
		Txs: []ethapi.CallArgs{callArgAddr1, callArgAddr2}}}, BundleContext{BlockNumberOrHash: rpc.BlockNumberOrHashWithNumber(rpc.LatestBlockNumber)}, &timeout)
	if err != nil {
		t.Errorf("calling intra bundle: %v", err)
	}
	...
	if addr1Balance != 100 || addr2Balance != 0 {
		t.Errorf("calling intra bundle: %v", "balanceUnmatch")
	}

	res, err = api.CallIntraBundle(ctx, []Bundle{{
		Txs: []ethapi.CallArgs{callArgAddr1, callArgAddr2}}}, BundleContext{BlockNumberOrHash: rpc.BlockNumberOrHashWithNumber(1), TransactionIndex: 2}, &timeout)
	if err != nil {
		t.Errorf("calling intra bundle: %v", err)
	}
	...
	if addr1Balance != 0 || addr2Balance != 100 {
		t.Errorf("calling intra bundle: %s", "balanceUnmatch")
	}

	res, err = api.CallIntraBundle(ctx, []Bundle{{Txs: []ethapi.CallArgs{callArgTransferAddr2, callArgAddr1, callArgAddr2}}}, BundleContext{BlockNumberOrHash: rpc.BlockNumberOrHashWithNumber(rpc.LatestBlockNumber)}, &timeout)
	if err != nil {
		t.Errorf("%v", err)
	}
